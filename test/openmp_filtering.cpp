...
struct RGB1 {
    unsigned char r, g, b;
    RGB1() : r(0), g(0), b(0) {}
    RGB1(unsigned char red, unsigned char green, unsigned char blue) : r(red), g(green), b(blue) {}
};

class SimpleImage1 {
private:
    std::vector<std::vector<RGB1>> pixels;
    int width, height;

public:
    SimpleImage1(int w, int h) : width(w), height(h) {
        pixels.resize(height, std::vector<RGB1>(width));
    }
    ...
    RGB1 getPixel(int x, int y) const {
    ...
    void setPixel(int x, int y, const RGB1& color) {
...
public:
    SimpleImage1 convolve(const SimpleImage1& input) {
        int width = input.getWidth();
        int height = input.getHeight();
        SimpleImage1 output(width, height);

        // Perform convolution on each RGB channel separately
        // Parallelize the outer loop (rows) using OpenMP
            ...
            for (int x = 0; x < width; x++) {
                ...
                for (int ky = -2; ky <= 2; ky++) {
                    for (int kx = -2; kx <= 2; kx++) {
                        ...
                        RGB1 pixelValue = input.getPixel(px, py);
...
int main() {
    // std::cout << "5x5 Gaussian Filter Convolution (Color Preserving) - OpenMP Parallelized\n";
    // std::cout << "========================================================================\n\n";
    ...
    const std::string inputJpg = "../images/large_image.jpg";
    const std::string outputJpg = "../results/omp_filtered_image.jpg";
    ...
    if (!SimpleImage1::convertJpgToPpm(inputJpg, tempPpm)) {
    ...
    SimpleImage1 image(100, 100); // Initial size, will be updated during load
    ...
    GaussianFilter1 filter;
    // filter.printKernel();

    // Apply convolution
    // std::cout << "Applying 5x5 Gaussian filter to RGB channels (parallel)...\n";
    auto start = std::chrono::high_resolution_clock::now();
    SimpleImage1 filtered = filter.convolve(image);
    ...
    if (!SimpleImage1::convertPpmToJpg(tempFilteredPpm, outputJpg)) {
